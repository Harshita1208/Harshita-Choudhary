class ProductGridModal extends HTMLElement {
  ...
  async addToCart() {
    ...
    const colorBtnselected = this.querySelector('.color-button.selected')?.textContent.trim();
    const selectedSizeText = this.querySelector('.custom-select .selected-option')?.textContent.trim();
    if (!variantId || !productId || !selectedSizeText || !colorBtnselected || selectedSizeText === "Choose your size") {
      this.showSizeError();
      return;
    }
